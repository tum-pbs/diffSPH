...
parser.add_argument('--timeLimit', '-t', type=float, default=1, help='Total simulation time')
...
simulationName = 'Still Water'
exportName = f'17_stillWater'
...
fluid_sdf = lambda points: sampleSDF(points, operatorDict['translate'](lambda x: getSDF('box')['function'](x, torch.tensor([L/2/aspect,L/2]).to(points.device)), torch.tensor([0,-L/2]).to(points.device)), invert = False)
...
triangle_sdf = operatorDict['translate'](lambda x: getSDF('triangle')['function'](x, 
    torch.tensor([-L/4,0]).to(device),
    torch.tensor([L/4,0]).to(device),
    torch.tensor([0,L/4]).to(device)), torch.tensor([-0.,-L/2]).to(device))

combined_sdf = lambda points: sampleSDF(points, operatorDict['union'](domain_sdf, triangle_sdf), invert = False)


config['particle']['shortEdge'] = True
regions = []

regions.append(buildRegion(sdf = combined_sdf, config = config, type = 'boundary', kind = 'constant'))
